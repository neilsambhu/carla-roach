...
def CheckScoreFiles():
	if os.path.exists("score_composed.txt") and os.path.exists("score_route.txt"):
		return True
	else:
		return False
...
if __name__ == '__main__':
	...
	for seed in [2021,2022,2023]:
		...
		# benchmarkProcess = subprocess.run(["python","-u","benchmark_NeilBranch0.py","resume=true","log_video=true","wb_project=iccv21-roach-benchmark",f'agent={PPO_exp_NCd_tt.agent}',f'actors.hero.agent={PPO_exp_NCd_tt.agent}',f'agent.ppo.wb_run_path={PPO_exp_NCd_tt.agent_ppo_wb_run_path}',f'wb_group=\"{PPO_exp_NCd_tt.wb_group}\"',f'wb_notes="{PPO_exp_NCd_tt.wb_notes}"',f'test_suites={PPO_exp_NCd_tt.test_suites}',f'seed={PPO_exp_NCd_tt.seed}',f'+wb_sub_group={PPO_exp_NCd_tt.wb_sub_group}',"no_rendering=true"],timeout=secondsIn365Days)
		# benchmarkProcess = subprocess.run(["python","-u","benchmark_NeilBranch0.py","resume=true","log_video=true","wb_project=iccv21-roach-benchmark",f'agent={PPO_exp_NCd_tt.agent}',
		# 	f'actors.hero.agent={PPO_exp_NCd_tt.agent}',f'agent.ppo.wb_run_path={PPO_exp_NCd_tt.agent_ppo_wb_run_path}',f'wb_group=\"{PPO_exp_NCd_tt.wb_group}\"',
		# 	f'wb_notes="{PPO_exp_NCd_tt.wb_notes}"',f'test_suites={PPO_exp_NCd_tt.test_suites}',f'seed={PPO_exp_NCd_tt.seed}',f'+wb_sub_group={PPO_exp_NCd_tt.wb_sub_group}',
		# 	"no_rendering=true"],shell=True,timeout=secondsIn365Days)
